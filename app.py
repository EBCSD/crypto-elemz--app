    ...
    if scan_all_pairs:
            ...
            try:
                ...
                if signal != "VÁRAKOZÁS":
                    ...
                    with st.expander(f"🔥 {sym} - JELZÉS: {signal}", expanded=True):
                        ...
                        col1.metric("Belépő", f"${entry:,.4f}")
                        col2.metric("Stop Loss", f"${sl:,.4f}")
                        col3.metric("Take Profit 1", f"${tp1:,.4f}")
                        col4.metric("Take Profit 2", f"${tp2:,.4f}")
                        st.info(f"📐 **Bitget méretezés:** Áttétel: **{lev}x** | Pozíció méret: **${pos_size:,.2f}**")
            except:
                continue
        ...
        if selected_pair and selected_pair != "Nincs adat":
            ...
            fig.add_trace(go.Candlestick(
                ...
                name="15M Gyertyák", increasing_line_color='#089981', decreasing_line_color='#f23645', increasing_fillcolor='#089981', decreasing_fillcolor='#f23645'
            ))
            ...
            fig.update_layout(
                template="plotly_dark", xaxis_rangeslider_visible=False, height=480, margin=dict(l=10, r=60, t=10, b=10),
                yaxis=dict(range=[y_min, y_max], fixedrange=False, side="right", gridcolor="#212529", tickfont=dict(size=12)), xaxis=dict(gridcolor="#212529"), showlegend=False
            )
